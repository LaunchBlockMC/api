    ...
    public Response toResponse(final Exception e) {
        ...
        if (e instanceof final NotAllowedException notAllowedException) {
            return Response.status(405)
                    .type(MediaType.APPLICATION_JSON_TYPE)
                    .entity(new ExceptionResponse("Method not allowed", new ErrorDetails("?", "Perhaps you're trying to do the wrong http method?", "", "api"), ExceptionMapper.getCurrentTraceId()))
                    .build();
        }
        ...
        if (e instanceof final NotFoundException notFoundException) {
            return Response.status(404)
                    .type(MediaType.APPLICATION_JSON_TYPE)
                    .entity(new ExceptionResponse("Method not found", new ErrorDetails("PS-NF", "That method doesn't exist, try the documentation?", "", "api"), ExceptionMapper.getCurrentTraceId()))
                    .build();
        }
        ...
        return Response.status(500)
                .type(MediaType.APPLICATION_JSON_TYPE)
                .entity(new ExceptionResponse("An unexpected error happened while processing this request", new ErrorDetails("?", "An unhandled error occurred", "", "api"), ExceptionMapper.getCurrentTraceId()))
                .build();
